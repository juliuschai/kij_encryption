...
class blowfish:
    ...
    def cipher(self, xl, xr, direction):
        if direction == 'encrypt':
            for i in range(16):
                xl = xl ^ self.P_BOXES[i]  # binary or with P_BOX
                ...
                # obtain the integer
                f = (int(self.S_BOXES[0][xl_div[0]]) + int(self.S_BOXES[1][xl_div[1]])) % MODULUS  # sum
                f = f ^ int(self.S_BOXES[2][xl_div[2]])  # binary xor
                f = ((f + int(self.S_BOXES[3][xl_div[3]])) % MODULUS) & 0xFFFFFFFF  # sum

                xr = f ^ xr  # binary or with right part
                xl, xr = xr, xl  # switch

            xl, xr = xr, xl  # switch
            xr = xr ^ self.P_BOXES[16]  # right part binary or with P_BOX 16
            xl = xl ^ self.P_BOXES[17]  # left part binary or with P_BOX 17
            ...
            xl, xr = xr, xl  # switch
            xr = xr ^ self.P_BOXES[1]  # right part binary or with P_BOX 1
            xl = xl ^ self.P_BOXES[0]  # left part binary or with P_BOX 0

        # end of 16 iterations
        return xl, xr

    def char_to_bin(char):
        return int(format(ord("a"), '08b'), 2)
    ...
    def encrypt(self, data):
        ...
        # big endian
        chars = ''.join([
            chr((cl >> 24) & 0b11111111), chr((cl >> 16) & 0b11111111), chr((cl >> 8) & 0b11111111),
            chr(cl & 0b11111111),
            chr((cr >> 24) & 0b11111111), chr((cr >> 16) & 0b11111111), chr((cr >> 8) & 0b11111111),
            chr(cr & 0b11111111)
        ])
        return chars
    ...
    def decrypt(self, data):
        ...
        xl, xr = self.cipher(cl, cr, 'decrypt')
